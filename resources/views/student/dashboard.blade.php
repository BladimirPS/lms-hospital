...
@section('content')
    ...
    <div class="grid grid-cols-1 gap-6 mb-8 md:grid-cols-3">
        ...
        <a href="{{ route('student.courses.completed') }}"
            ...
            <div class="p-3 rounded-full" style="background-color: #EDFAF1;">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor" style="color: #27AE60;">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <a href="{{ route('student.diplomas') }}"
            ...
            <div class="p-3 rounded-full" style="background-color: #FFF3E0;">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor" style="color: #E67E22;">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex items-center justify-between p-6 mb-4 bg-white shadow-sm rounded-xl">
                    <div class="flex-1">
                        ...
                        <div class="mt-2">
                            <div class="w-full h-2 bg-gray-200 rounded-full">
                                <div class="h-2 rounded-full" style="width: 100%; background-color: #27AE60;">
                                </div>
                            </div>
                    ...
                    <div class="flex gap-2 ml-6">
                        ...
                        @if($enrollment->diploma)
                            <a href="{{ route('student.diplomas') }}" class="px-4 py-2 text-sm font-medium text-white rounded-lg"
    ...
    <span class="px-2 py-1 text-xs font-semibold rounded-full"
          style="background-color: #FEE2E2; color: #E74C3C;">
        🔒 Vencido
    </span>
@elseif($enrollment->getEffectiveDueDate() && $enrollment->getEffectiveDueDate()->diffInDays(now()) <= 7)
    <span class="px-2 py-1 text-xs font-semibold rounded-full"
          style="background-color: #FFF3E0; color: #E67E22;">
        ⚠️ Vence pronto
    </span>
@elseif($enrollment->getEffectiveDueDate())
    <span class="px-2 py-1 text-xs font-semibold rounded-full"
          style="background-color: #EDFAF1; color: #27AE60;">
        ✓ Disponible
    </span>
@endif
                    ...
                    <h3 class="font-semibold text-gray-800">{{ $enrollment->course->title }}</h3>
                    @if($enrollment->getEffectiveDueDate())
                        <p class="mt-1 text-xs text-gray-400">
                            Vence el {{ $enrollment->getEffectiveDueDate()->format('d/m/Y') }}
                        </p>
                    @endif
                ...
                <div class="ml-6">
                    @if($enrollment->is_locked)
                        <span class="px-4 py-2 text-sm font-medium text-gray-400 bg-gray-100 rounded-lg">
                            🔒 Vencido
                        </span>
                    @elseif($enrollment->is_not_started)
                        <span class="px-4 py-2 text-sm font-medium text-gray-400 bg-gray-100 rounded-lg">
                            🕐 {{ $enrollment->getEffectiveStartDate()?->format('d/m/Y') }}
                        </span>
                    @else
                        <a href="{{ route('student.course.show', $enrollment) }}"
